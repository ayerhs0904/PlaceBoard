    ...
    <div className="min-h-screen bg-[#050510] text-white">
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-8 mt-6">
          ...
          {!resumeUrl || isUpdatingResume ? (
            ...
            <div className="flex items-center justify-between bg-white/5 p-4 rounded-xl border border-white/10">
              <div className="flex items-center gap-3 overflow-hidden">
                <LinkIcon className="text-slate-400 shrink-0" />
                <span className="text-slate-300 truncate">{resumeUrl}</span>
              </div>
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              <Sparkles className="text-violet-400" /> AI Resume Tips
            </h2>
            <button 
              onClick={generateTips}
              disabled={loadingTips}
              className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-6 py-2 rounded-xl font-bold hover:from-violet-700 hover:to-indigo-700 transition-all disabled:opacity-50 flex items-center gap-2 shadow"
            >
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl">
          ...
          {skillsGap.length === 0 ? (
            <p className="text-slate-400 text-center py-8">Apply to more roles to see skills gap analysis.</p>
          ) : (
            <div className="space-y-4">
              {skillsGap.map((gap, i) => (
                <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-5">
                  <h3 className="font-bold text-lg mb-3 text-violet-300">{gap.role}</h3>
